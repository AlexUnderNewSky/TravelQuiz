...
namespace PolyNav
{
    ...
    public class PolyNavAgent : MonoBehaviour
    {
        ...
        void LateUpdate() {
            ...
            //rotate if must
			atanToTarget = -Mathf.Atan2(movingDirection.x, movingDirection.y) * 180 / Mathf.PI;

            if ( rotateTransform ) {
				float newZ = Mathf.MoveTowardsAngle(transform.localEulerAngles.z, atanToTarget, rotateSpeed * Time.deltaTime);
