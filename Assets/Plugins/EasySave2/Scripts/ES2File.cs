...
public static class ES2File
{
	public static void Delete(ES2Settings settings)
	{
		...
		if(settings.saveLocation == ES2Settings.SaveLocation.Resources)
			Debug.LogError("You can not delete from Resources.");
		...
		if(!settings.filenameData.HasTag())
		{
			...
			if(settings.saveLocation == ES2Settings.SaveLocation.PlayerPrefs)
			{
				if(settings.filenameData.IsFile())
					...
					Debug.LogError("You cannot delete folders using Easy Save 2 for PlayerPrefs.");
			}
			// File
	...
	public static bool Exists(ES2Settings settings)
	{
		...
		if(settings.saveLocation == ES2Settings.SaveLocation.Resources)
		{
			if(settings.filenameData.extension != ".bytes")
				Debug.LogError("Can only check existence of files from Resources with the extension '.bytes'.");
	...
	public static void Rename(ES2Settings settings, ES2Settings newSettings)
	{
		// Resources
		if(settings.saveLocation == ES2Settings.SaveLocation.Resources)
			Debug.LogError("You can not rename a file in Resources.");
		...
		if(!settings.filenameData.HasTag())
		{
			...
			if(settings.saveLocation == ES2Settings.SaveLocation.PlayerPrefs)
			{
				if(settings.filenameData.IsFile())
				{
					...
					Debug.LogError("You cannot rename folders using Easy Save 2 for PlayerPrefs.");
			}
			// File
			else if(settings.saveLocation == ES2Settings.SaveLocation.File)
			{
	...
	public static string[] GetFolders(ES2Settings settings)
	{
		if(settings.saveLocation == ES2Settings.SaveLocation.Resources || settings.saveLocation == ES2Settings.SaveLocation.PlayerPrefs)
			Debug.LogError("You can only use GetFolders when using the File save location.");
			
		if(!settings.filenameData.IsFolder())
			Debug.LogError("You should only provide GetFolders with a path to a folder, not a file or tag.");
	...
	{
		if(settings.saveLocation == ES2Settings.SaveLocation.Resources || settings.saveLocation == ES2Settings.SaveLocation.PlayerPrefs)
			Debug.LogError("You can only use GetFiles when using the File save location.");
			
		if(!settings.filenameData.IsFolder())
			Debug.LogError("You should only provide GetFiles with a path to a folder, not a file or tag.");
