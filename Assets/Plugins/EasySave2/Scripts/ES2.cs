...
public static class ES2 
{
	...
	public static void SaveImage(Texture2D tex, string identifier)
	{
		...
		if(newSettings.filenameData.extension.ToLower() != ".png")
			Debug.LogError("ES2.SaveImage can only be used to save PNG files. Please change your path extension to PNG.");
	...
	public static Texture2D LoadImage(string path)
	{
		if(Application.platform == RuntimePlatform.WebGLPlayer)
			Debug.LogError("You cannot use LoadImage with WebGL");
	...
	public static AudioClip LoadAudio(string path)
	{
		...
		if(Application.platform == RuntimePlatform.WebGLPlayer)
			Debug.LogError("You cannot use LoadAudio with WebGL.");
		...
		if(settings.filenameData.extension.ToLower() == ".mp3" 
		   ...
		   || Application.platform == RuntimePlatform.OSXPlayer))
			Debug.LogError("You can only load Ogg, WAV,XM, IT, MOD or S3M with Unity Standalone");

		if(settings.filenameData.extension.ToLower() == ".ogg" 
		   && (Application.platform == RuntimePlatform.IPhonePlayer
		    || Application.platform == RuntimePlatform.Android))
			Debug.LogError("You can only load MP3, WAV,XM, IT, MOD or S3M with Unity on Mobile Platforms");
