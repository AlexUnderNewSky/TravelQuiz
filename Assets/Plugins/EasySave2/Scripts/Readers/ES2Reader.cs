...
public partial class ES2Reader : System.IDisposable
{
	...
	public T[,] Read2DArray<T>(ES2Type type)
	{
		...
		if(dimensionCount != 2)
			Debug.LogError("The array you are loading with ES2.Load2DArray must have 2 dimensions, not "+dimensionCount+" dimensions.");
	...
	public T[,,] Read3DArray<T>(ES2Type type)
	{
		...
		// If array we are loading does not have 2 dimensions, throw error.
		if(dimensionCount != 3)
			Debug.LogError("The array you are loading with ES2.Load3DArray must have 3 dimensions, not "+dimensionCount+" dimensions.");
	...
	protected void ProcessHeader(ES2Keys.Key expectedCollectionType, ES2Type expectedValue, ES2Type expectedKey, string tag)
	{
		if(ScanToTag(tag) == false)
			Debug.LogError("The data, tag, file or folder you are looking for does not exist. Please ensure that ES2.Exists(string identifier) returns true before calling this method.");
		...
		if(expectedCollectionType != header.collectionType)
		{
			...
			if(expectedCollectionType == ES2Keys.Key._Null)
				Debug.LogError("The data you are trying to load is a Collection. Please use the Load method for that type of collection (for example, ES2.LoadArray or ES2.LoadDictionary)");
			else
				Debug.LogError("The data you are trying to load is not a Collection, but you are using a Collection method to use it. Use ES2.Load instead.");
		}
		
		if(expectedKey != null)
			if(header.keyType != expectedKey.hash)
				Debug.LogError("The type of key in the Dictionary you are loading does not match the key you are trying to load with.");

#if UNITY_EDITOR
        if (expectedValue != null)
			if(header.valueType != expectedValue.hash)
				Debug.LogError("The data you are trying to load does not match the Load method you are using to load it. ");
#endif
...
{
	public ES2InvalidDataException() : base("The file provided does not contain data that is readable by Easy Save. Please make sure that file was created by Easy Save.")
	{
	}
}
