	...
	public void Init(string path, ES2Settings settings, bool useParameters)
	{
		...
		if(split.Length == 2)
		{
			...
			if(tagSplit.Length % 2 !=0)
				Debug.LogError("One or more parameters are missing a value, or parameter value contains invalid character such as '=' or '&'.");
			...
			Debug.LogError("There should be no more than one '?' character in your filepath. Please check that you have not accidentally put in more than one.");
	}
		...
		switch(name)       
      	{         
        	...
        	case "id": // ID
        		break;
        	default:
        		Debug.LogError("Unknown parameter '"+name+"'.");	
				break;
		}
	}
